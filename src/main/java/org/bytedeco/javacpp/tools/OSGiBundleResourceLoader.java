...
public class OSGiBundleResourceLoader {
	...
	public static String getContainerBundleName(URL resourceURL) {
		requireOSGi();
		return Loader.getContainerBundleName(resourceURL);
	}

	public static Enumeration<URL> getBundleDirectoryContent(URL resourceURL) {
		requireOSGi();
		return Loader.getBundleDirectoryContent(resourceURL);
	}

	private static void requireOSGi() {
		if (!IS_OSGI_RUNTIME) {
			throw new IllegalStateException(
				OSGiBundleResourceLoader.class.getSimpleName() + " must only be used in a OSGi runtime");
		}
	}
		...
		private static BundleContext getBundleContext() {
			...
			if (bundle != null) {
				...
				if (state != Bundle.ACTIVE) {
					// FIXME: check the exact state?!
					if (state == Bundle.INSTALLED || state == Bundle.RESOLVED || state == Bundle.STARTING) {
						try {
							bundle.start();
